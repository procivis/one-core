use one_core::repository::{
    data_provider::{GetProofSchemaQuery, GetProofSchemaResponse, SortableProofSchemaColumn},
...
use crate::{
    ...
    data_model::proof_schema_from_model,
    entity::{proof_schema, ProofSchema},
        ...
        Ok(GetProofSchemaResponse {
            values: proof_schemas
                ...
                .map(|proof_schema| {
                    ...
                    proof_schema_from_model(proof_schema, claims)
                })
                .collect(),
...
mod tests {
    ...
    async fn test_get_proof_schemas_simple() {
        ...
        let result = data_layer
            .get_proof_schemas(from_pagination(0, 1, organisation_id.to_owned()))
            .await;
    ...
    async fn test_get_proof_schemas_empty() {
        ...
        let result = data_layer
            .get_proof_schemas(from_pagination(0, 1, Uuid::new_v4().to_string()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
    ...
    async fn test_get_proof_schemas_pages() {
        ...
        let result = data_layer
            .get_proof_schemas(from_pagination(0, 10, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        ...
        let result = data_layer
            .get_proof_schemas(from_pagination(1, 10, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        assert_eq!(50, response.total_items);
        ...
        let result = data_layer
            .get_proof_schemas(from_pagination(5, 10, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        assert_eq!(50, response.total_items);
    ...
    async fn test_get_proof_schemas_sorting() {
        ...
        let result = data_layer
            .get_proof_schemas(from_pagination(0, 2, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        assert_eq!(2, response.total_items);

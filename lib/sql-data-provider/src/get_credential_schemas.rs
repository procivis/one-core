use one_core::repository::data_provider::{
    GetCredentialClaimSchemaResponse, GetCredentialSchemaQuery, SortableCredentialSchemaColumn,
};
...
use crate::data_model::credential_schema_response_from_model;
        ...
        Ok(GetCredentialClaimSchemaResponse {
            values: schemas
                ...
                .map(|credential_schema| {
                    credential_schema_response_from_model(credential_schema, &claims)
                })
                .collect(),
...
mod tests {
    ...
    async fn test_get_credential_schemas_empty() {
        ...
        let result = data_layer
            .get_credential_schemas(from_pagination(0, 1, Uuid::new_v4().to_string()))
            .await;
    ...
    async fn test_get_credential_schemas_pages() {
        ...
        let result = data_layer
            .get_credential_schemas(from_pagination(0, 10, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        ...
        let result = data_layer
            .get_credential_schemas(from_pagination(1, 10, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        assert_eq!(50, response.total_items);
        ...
        let result = data_layer
            .get_credential_schemas(from_pagination(5, 10, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        assert_eq!(50, response.total_items);
    ...
    async fn test_get_credential_schemas_sorting() {
        ...
        let result = data_layer
            .get_credential_schemas(from_pagination(0, 2, organisation_id.to_owned()))
            .await;
        assert!(result.is_ok());
        let response = result.unwrap();
        assert_eq!(2, response.total_items);

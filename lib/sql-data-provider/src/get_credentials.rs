use one_core::repository::data_provider::{
    DetailCredentialResponse, GetCredentialsQuery, GetCredentialsResponse, SortableCredentialColumn,
};
...
use crate::common_queries::fetch_claim_claim_schemas;
use crate::data_model::{
    detail_credential_from_combined_credential_did_and_credential_schema,
    CredentialDidCredentialSchemaCombined,
};
use crate::entity::{credential, credential_schema, credential_state, did, Credential};
use crate::list_query::{GetEntityColumn, SelectWithListQuery};
...
async fn combine_with_claims(
    ...
    let result = combined_credentials
        .into_iter()
        .map(|value| {
            detail_credential_from_combined_credential_did_and_credential_schema(value, &claims)
        })
...
mod tests {
    ...
    use crate::{
        common_queries::insert_credential_state, entity::credential_state,
        list_query::from_pagination, test_utilities::*, OldProvider,
    };
    ...
    async fn get_credentials_test_simple() {
        ...
        let credentials = test_data
            .data_layer
            .get_credentials(from_pagination(0, 2, test_data.organisation_id.to_owned()))
            .await;

...
use one_core::repository::data_provider::{GetProofsQuery, GetProofsResponse, SortableProofColumn};

use crate::data_model::proof_detail_response_from_models;

use super::common::calculate_pages_count;
use super::data_model::ProofsCombined;
use super::entity::{did, proof, proof_schema, proof_state, Proof, ProofState};
use super::list_query::{GetEntityColumn, SelectWithListQuery};
use super::{DataLayerError, OldProvider};
...
impl OldProvider {
    pub async fn get_proofs(
        ...
        Ok(GetProofsResponse {
            values: models
                .into_iter()
                .map(proof_detail_response_from_models)
                .collect(),
...
mod tests {
    ...
    use crate::{
        common_queries::insert_proof_state, data_model::convert_proof_state, entity::proof_state,
    ...
    async fn get_proofs_test_simple() {
        ...
        let proofs = test_data
            .data_layer
            .get_proofs(from_pagination(0, 10, test_data.organisation_id.to_owned()))
            .await;
        assert!(proofs.is_ok());
        ...
        for (proof_id, verifier_did, state, schema_id) in test_data.proofs {
            ...
            assert_eq!(proof.state, convert_proof_state(state));

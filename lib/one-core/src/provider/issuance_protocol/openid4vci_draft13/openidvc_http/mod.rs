...
impl OpenID4VP20HTTP {
    ...
    pub(crate) async fn holder_handle_invitation(
        ...
        if !self.can_handle(&url) {
            return Err(IssuanceProtocolError::Failed(
                "No OpenID4VCI query params detected".to_string(),
            ));
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn holder_accept_credential(
        &self,
    ...
    pub(crate) async fn holder_reject_credential(
        &self,
    ...
    pub(crate) async fn issuer_share_credential(
        &self,
        credential: &Credential,

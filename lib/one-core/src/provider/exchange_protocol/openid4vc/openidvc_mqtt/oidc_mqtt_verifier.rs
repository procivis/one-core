...
fn wallet_connect(
    topics: Topics,
    keypair: KeyAgreementKey,
) -> BoxFuture<'static, Result<MQTTVerifierContext, ExchangeProtocolError>> {
    async {
        ...
        let identity_request =
            parse_identity_request(identify_bytes).map_err(ExchangeProtocolError::Transport)?;
...
fn receive_presentation(
    context: Arc<MQTTVerifierContext>,
) -> BoxFuture<'static, Result<MQTTOpenId4VpResponse, ExchangeProtocolError>> {
    (async move {

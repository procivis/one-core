...
pub(super) struct JsonLd {
    pub base_url: Option<String>,
    pub crypto: Arc<dyn CryptoProvider>,
}

impl JsonLd {
    pub(super) fn new(crypto: Arc<dyn CryptoProvider>, base_url: Option<String>) -> Self {
        Self { crypto, base_url }
    }
    ...
    pub(super) fn prepare_context(&self, additional_context: Vec<String>) -> Vec<String> {
        let mut context = vec![
    ...
    pub(super) async fn canonize_any<T>(&self, json_ld: &T) -> Result<String, FormatterError>
    where
        T: Serialize,
    {
    ...
    pub(super) async fn canonize_dataset(
        &self,
        dataset: LdDataset,
    ) -> Result<String, FormatterError> {

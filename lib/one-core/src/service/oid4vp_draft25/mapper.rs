...
pub(super) fn parse_interaction_content(
    data: Option<&Vec<u8>>,
) -> Result<OpenID4VPVerifierInteractionContent, OID4VPDraft25ServiceError> {
    if let Some(interaction_data) = data {
        serde_json::from_slice(interaction_data)
            .map_err(|e| OID4VPDraft25ServiceError::MappingError(e.to_string()))
    } else {
        Err(OID4VPDraft25ServiceError::MappingError(

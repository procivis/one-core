...
async fn insert_intermediary_array_claims(
    ...
    for array_claim_schema in &array_claim_schemas {
        ...
        for credential_id in credential_ids {
            let result =
                migration_credential_arrays(db, backend, array_claim_schema, &credential_id).await;
            if let Err(err) = result {
                tracing::warn!(
                    "Failed to migrate array claims of credential {credential_id}: {err}"
                )
            }
        }
    }
...
async fn migration_credential_arrays(
    db: &SchemaManagerConnection<'_>,
    backend: DbBackend,
...
async fn insert_intermediary_object_claims(
    ...
    for object_claim_schema in &object_claim_schemas {
        ...
        // Migrate each credential
        for credential_id in credential_ids {
            let result = migrate_credential_objects(
                db,
                backend,
                object_claim_schema,
                &child_claim_schema_ids,
                &credential_id,
            )
            .await;
            if let Err(err) = result {
                tracing::warn!(
                    "Failed to migrate object claims of credential {credential_id}: {err}"
                )
            }
        }
    }
...
async fn migrate_credential_objects(
    db: &SchemaManagerConnection<'_>,
    backend: DbBackend,
    object_claim_schema: &ClaimSchemaResult,
    child_claim_schema_ids: &[IdResult],
    credential_id: &str,
) -> Result<(), DbErr> {
    ...
    for child_claim_schema_id in child_claim_schema_ids.iter() {
        ...
        for direct_child_candidate in results {
            if !child_claims
                .iter()
                .any(|child: &ClaimIdPath| direct_child_candidate.path.starts_with(&child.path))
            {

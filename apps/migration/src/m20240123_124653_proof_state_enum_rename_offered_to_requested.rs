        ...
        match manager.get_database_backend() {
            sea_orm::DatabaseBackend::Postgres => {
                manager
                    .exec_stmt(
                        Type::alter()
                            .rename_value(ProofRequestState::Requested, ProofRequestState::Offered)
                            .to_owned(),
                    )
                    .await?;
            }
            sea_orm::DatabaseBackend::MySql => {
                ...
                // Rename requested enum entries to offered
                manager
                    .exec_stmt(
                        Query::update()
                            .table(ProofState::Table)
                            ...
                            .and_where(
                                Expr::col(ProofState::State).eq(ProofRequestState::Requested
                                    .as_expr()
                                    .as_enum(ProofRequestStateEnum)),
                            )
                            .to_owned(),
            ...
            sea_orm::DatabaseBackend::Sqlite => {
                manager
                    .exec_stmt(
                        Query::update()
                            .table(ProofState::Table)
                            .value(ProofState::State, ProofRequestState::Offered.as_expr())
                            .and_where(
                                Expr::col(ProofState::State)
                                    .eq(ProofRequestState::Requested.as_expr()),
                            )
                            .to_owned(),
                    )
                    .await?;

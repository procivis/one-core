...
#[tokio::test]
async fn test_get_issuer_configuration_draft13_swiyu() {
    // GIVEN
    let listener = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
    let base_url = format!("http://{}", listener.local_addr().unwrap());
    let config = fixtures::create_config(&base_url, None);
    ...
    let url = format!(
        "{base_url}/ssi/openid4vci/draft-13-swiyu/{}/.well-known/openid-configuration",
        credential_schema.id
    );
    let resp = utils::client().get(url).send().await.unwrap();
    ...
    let issuer = format!(
        "{base_url}/ssi/openid4vci/draft-13-swiyu/{}",
        credential_schema.id
    );

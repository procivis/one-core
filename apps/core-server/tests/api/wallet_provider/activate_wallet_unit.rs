...
#[tokio::test]
async fn activate_wallet_unit_nonce_invalid() {
    // given
    let (context, org) = TestContext::new_with_organisation(None).await;
    create_wallet_unit_attestation_issuer_identifier(&context, &org).await;
    ...
    let resp = context
        .api
        .wallet_provider
        .activate_wallet(wallet_unit.id, "dummy attestation", "invalid-nonce")
        .await;
...
async fn activate_wallet_unit_invalid_id() {
    ...
    let resp = context
        .api
        .wallet_provider
        .activate_wallet(Uuid::new_v4().into(), "dummy attestation", "nonce")
        .await;

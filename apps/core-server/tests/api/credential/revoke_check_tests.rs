...
async fn test_revoke_check_success() {
    ...
    let _revocation_list_mock = mock_server
        .mock_async(|when, then| {
            when.method(GET)
                .path("/ssi/revocation/v1/list/8bf6dc8f-228f-415c-83f2-95d851c1927b");
            then.status(200).body(status_list_credential_jwt);
        })
        .await;
    // WHEN
    ...
    //assert!(resp[0]["reason"].is_none());
    // TODO: Will be enabled after Mite takes a look on mocking host and port
}
